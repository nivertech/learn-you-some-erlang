...
%% formward a client's offer cancellation
...
%% Own side retracing an item offer
...
wait({do_offer, Item}, S=#state{otheritems=OtherItems}) ->
    ...
    notice(S, "other side offeering ~p", [Item]),
    {next_state, negotiate, S#state{otheritems=add(Item, OtherItems)}};
wait({undo_offer, Item}, S=#state{otheritems=OtherItems}) ->
    gen_fsm:reply(S#state.from, offer_changed),
    notice(S, "Other side cancelling offer for ~p", [Item]),
    {next_state, negotiate, S#state{otheritems=remove(Item, OtherItems)}};
...
%% but for our puroses, outputting to the shell is enough.
